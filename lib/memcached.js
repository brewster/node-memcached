...
(function (nMemcached) {
  ...
  memcached.command = function memcachedCommand(queryCompiler, server) {
    ...
    // Set up a timeout for connections
    var that = this;
    var hasTimedOut = false;
    var timeoutId = setTimeout(function () {
      hasTimedOut = true;
      // Record the number of timeouts for the given server
      if (!memcached.timeouts[server]) {
        memcached.timeouts[server] = 1;
      } else {
        memcached.timeouts[server] += 1;
      }

      // If we've exceeded our number of allowed timeouts, 
      // remove the server and cycle in a backup if possible
      if (memcached.timeouts[server] >= memcached.failures) {
        memcached.emit('remove', [server]);
        if (memcached.connections[server]){
          memcached.connections[server].end();
        }
        
        if (that.failOverServers && that.failOverServers.length) {
          memcached.HashRing.replaceServer(server, that.failOverServers.shift());
        } else {
          memcached.HashRing.removeServer(server);
          memcached.emit('failure', ['connection timeout']);
        }

        return query.callback && memcached.makeCallback(query.callback," <<-- CONNECTION TIMEOUT AT SERVER " + server + "! CYCLING SERVER OUT OF POOL -->>");
      } else {
        return query.callback && memcached.makeCallback(query.callback," <<-- connection timeout at server " + server + "! " + (memcached.failures - memcached.timeouts[server]) + " timeouts left before cycling. -->>");
      }
    }, memcached.timeout);    
